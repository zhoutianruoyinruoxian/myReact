...
export default class Gantt {
  ...
  get_date_info(date, last_date, i) {
    ...
    const x_pos = {
      ...
      Month_upper: this.options.column_width * 24 / 2
    };
  ...
  /**
   * @desc 在classNameList里查到element的index，获取tasks列表里对应item的disabled值，以此判断是否禁用当前元素
   * @author add by 周天 2018-9-6
   */
  disabled(classNameList, element) {
    let className = classNameList;
    let doms = [];
    if ((className).indexOf(',') > -1) {
      className = className.split(',');
      className.forEach(o => {
        doms = doms.concat([...document.getElementsByClassName(o)]);
      })
    } else {
      doms = [...document.getElementsByClassName(className)];
    }
    const index = doms.findIndex(o => o === element);
    if (index === -1) throw new Error('can not found element in classNameList')
    return this.tasks[index].disabled;
  }
  ...
  bind_bar_events() {
    ...
    $.on(this.$svg, 'mousedown', '.bar-wrapper, .handle', (e, element) => {
      //禁用操作
      const disabled = this.disabled('bar-wrapper', $.closest('.bar-wrapper', element));
      if (disabled) return;
    ...
    $.on(this.$svg, 'mousemove', e => {
      ...
      bars.forEach(bar => {
        ...
        bar.date_changed({ changing: true });
      });
    });
    ...
    $.on(this.$svg, 'mouseup', e => {
      ...
      bars.forEach(bar => {
        ...
        bar.date_changed({ changing: false });
  ...
  get_snap_position(dx) {
    ...
    if (this.view_is('Week')) {
      ...
      position =
        odx -
        rem +
        (rem < this.options.column_width / 48
          ? 0
          : this.options.column_width / 24);
    } else {
